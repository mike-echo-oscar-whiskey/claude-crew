...
5. **Register tasks in dependency order** so `Blocked by` can reference real numbers: for each task write its body to a temp file and run `${CLAUDE_PLUGIN_ROOT}/scripts/tracker.sh task create --story <n> --title "<title>" --role <role> --body-file <file> [--blocked-by "<numbers>"]`. The story's checklist is updated by the script. Task bodies are GitHub Markdown, never a wall of `Label: text` lines: a one-line header (task id · story · wave · role · design path), then `## Goal`, `## Likely files` (bullet list, every path in backticks), `## Tests` (numbered, the RED test first and marked, every test name in backticks), `## Done when` (bullets, one check per bullet, symbols and commands in backticks), and the common definition-of-done as one italic line. Story bodies keep the refine format (Waarom/Wat/Acceptatiecriteria...).
