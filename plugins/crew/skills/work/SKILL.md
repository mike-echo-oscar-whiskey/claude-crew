...
4. **Review.** In parallel: `crew:qa-engineer` reviews the tests, checks the red-run evidence and runs the profile's test command; `crew:security-engineer` when the task or story touches auth, tenants, secrets, outbound calls or untrusted input; the profile's specialist when the change crosses their domain. Feed findings back to the owning role once; a second round goes to the user.
